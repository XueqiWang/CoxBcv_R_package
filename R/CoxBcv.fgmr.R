...
CoxBcv.fgmr <- function(Y,Delta,X,ID){
  ...
  nom <- nomMR <- nomFGMR <- pracma::zeros(nbeta,n)
  ...
  for(i in UID){
    ...
    for (k in 1:nbeta){
      ...
      tempr1 <- pracma::zeros(1,nbeta)
      for (s in 1:nbeta){
        tempdd <- (pracma::repmat(X_c[,s,drop=FALSE],1,ny)-pracma::repmat(S1beta[s,,drop=FALSE]/t(S0beta),ny_c,1))*ylxb_c
        tempr1[1,s] <- sum(as.matrix(tempk*tempdd)%*%pracma::repmat(1,ny,1))
      ...
      nomMR2 <- sum(as.matrix(tempk*IndYY_c*exp(Xbeta_c)/pracma::repmat(S0beta,ny_c,1))%*%epsilon_c_all)
      nomMR[k,i] <- nomMR1+nomMR2
    }
    
    # components for FG type correction
    Hi <- pracma::zeros(nbeta,nbeta)
